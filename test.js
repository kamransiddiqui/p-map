...
test('returns fulfilled settled results when throwOnError is false and there are no errors', async t => {
	const result = await pMap(sharedInput, mapper, {stopOnError: false, throwOnError: false});
	t.deepEqual(result, [
...
test('throwOnError: true (default) still throws AggregateError when stopOnError is false', async t => {
	await t.throwsAsync(pMap(errorInput1, mapper, {concurrency: 1, stopOnError: false}), {instanceOf: AggregateError});
});
...
test('asyncIterator - returns fulfilled settled results when throwOnError is false and there are no errors', async t => {
	const result = await pMap(new AsyncTestData(sharedInput), mapper, {stopOnError: false, throwOnError: false});
	t.deepEqual(result, [
		{status: 'fulfilled', value: 10},
		{status: 'fulfilled', value: 20},
		{status: 'fulfilled', value: 30},
