...
test('returns settled results when throwOnError is false and stopOnError is false', async t => {
...
test('returns settled results when throwOnError is false and stopOnError is true', async t => {
...
test('returns fulfilled results when throwOnError is false and no errors', async t => {
	const result = await pMap(sharedInput, mapper, {concurrency: 1, throwOnError: false});
	t.is(result.length, 3);
...
test('throwOnError false with pMapSkip', async t => {
	const result = await pMap([
		1,
		pMapSkip,
		2,
		3,
	], async value => value, {throwOnError: false});
	t.is(result.length, 3);
	t.deepEqual(result[0], {status: 'fulfilled', value: 1});
	t.deepEqual(result[1], {status: 'fulfilled', value: 2});
	t.deepEqual(result[2], {status: 'fulfilled', value: 3});
});
...
test('asyncIterator - returns settled results when throwOnError is false and stopOnError is false', async t => {
...
test('asyncIterator - returns settled results when throwOnError is false and stopOnError is true', async t => {
...
test('asyncIterator - returns fulfilled results when throwOnError is false and no errors', async t => {
	const result = await pMap(new AsyncTestData(sharedInput), mapper, {concurrency: 1, throwOnError: false});
	t.is(result.length, 3);
	t.deepEqual(result[0], {status: 'fulfilled', value: 10});
	t.deepEqual(result[1], {status: 'fulfilled', value: 20});
	t.deepEqual(result[2], {status: 'fulfilled', value: 30});
...
test('asyncIterator - throwOnError false with pMapSkip', async t => {
	const result = await pMap(new AsyncTestData([
		1,
		pMapSkip,
		2,
		3,
